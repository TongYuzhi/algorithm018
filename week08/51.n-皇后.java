...
class Solution {
    ...
    private void solveNQueens(int n, int[] queens, int row,
        ...
        if (row == n) {
            res.add(paint(n, queens));
            return;
        }
        for (int i = 0; i < n; i++) {
            if (columns.contains(i) || leDiagonals.contains(row + i) || riDiagonals.contains(row - i)) { 
                continue;
            }
            queens[row] = i;
            columns.add(i);
            leDiagonals.add(row + i);
            riDiagonals.add(row - i);
            solveNQueens(n, queens, row + 1, res, columns, leDiagonals, riDiagonals);
            riDiagonals.remove(row - i);
            leDiagonals.remove(row + i);
            columns.remove(i);
            queens[row] = -1;
        }
    }

    private List<String> paint(int n, int[] queens) {
